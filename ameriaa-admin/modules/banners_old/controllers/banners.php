...
* @category   Banners
* @package    Banners
* @author     sankar <sanakr.g@siriinnovations.com>
...
class Banners extends Controller {
    ...
    */
    ...
    public function index() {  
        ...
        $this->view->banner             = $this->model->processGetBanners();    // get banners list
        $this->view->LoadView('banners_view', 'banners');
    } 
    ...
    public function add() { 
        ...
        $this->view->LoadView('banners_add', 'banners');
    }
    ...
    public function addbanner() {
        ...
        if(!empty($_FILES['image_file']['name'])){
            ...
            if($imageWidth == $orgDim[0] && $imageHeight == $orgDim[1]) {            
                ...
                if($result != 0){
                    $this->session->sets('addSuccess' ,'New banner details are sucessfully added');
                    $this->redirect('banners');
                } else {
                    $this->session->sets('addFailure' ,'Please check the details');
    ...
    public function edit($id) {
        ...
        $this->view->LoadView('banners_edit', 'banners');        
    }
    ...
    public function updateBanner() {
        ...
        if(!empty($_FILES['image_file']['name'])){
            ...
            if($imageWidth == $orgDim[0] && $imageHeight == $orgDim[1]) {   
                ...
                $this->session->sets('image_dimension' ,'Please upload '.$originalDimensions['position_size'].' dimension image');
                $this->redirect("banners/edit/".$_POST['banner_id']);
                exit;
            }
        ...
        if($result) {
            $this->session->sets('EditSucess', 'Banner details are sucessfully updated');
            $this->redirect("banners");
        } else {
