...
//    }
    ...
    public function updatedetails() {        
        $filedir  = ADMINBASE . "uploads/members/";
        $thumbdir = ADMINBASE . "uploads/members/thumbs/";        
        
        // image upload 
        if(!empty($_FILES['image']['name'])) {            
            $randName = rand(101010, 909090);
            $newName = $randName."_member_pic";
        ...
        // file update
        if(!empty($_FILES['licence_attach']['name'])) {            
            $randName = rand(101010, 909090);
            $newName = $randName."_attachment";
            $ext = substr($_FILES['licence_attach']['name'], strrpos($_FILES['licence_attach']['name'], '.') + 1);
            $this->ImageUpload->File      = $_FILES['licence_attach'];
            $this->ImageUpload->method    = 1;
            $this->ImageUpload->SavePath  = $filedir;
            $this->ImageUpload->NewName   = $newName;
            $this->ImageUpload->OverWrite = true;
            $err = $this->ImageUpload->UploadFile();            
            $_POST['licence_attach'] = $newName.".".strtolower($ext);
        }
        //
        
        $result = $this->model->updateDetails($_POST);
        if($result) {
            $this->session->sets("success", 'Profile details successfully updated');
